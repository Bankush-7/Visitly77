...
module.exports.createListing = async (req, res, next) => {
  ...
  let imageUrls = [];
  if (req.files && req.files.image) {  // Corrected to 'image' singular
    imageUrls = req.files.image.map(file => ({
      url: file.path,
  ...
  let commercialv = req.body.listing.commercial === "true";

  // Create a new Listing instance
  const newListing = new Listing(req.body.listing);
  newListing.owner = req.user._id;
  newListing.image = imageUrls; // Store the images as an array
  newListing.geometry = response.body.features[0].geometry;
  newListing.sustainable = sustainablev;
  newListing.commercial = commercialv;
...
module.exports.renderEditForm = async (req, res) => {
  ...
  let originalImageUrls = listing.image && listing.image.length > 0
    ? listing.image.map(image => image.url.replace("/upload", "/upload/w_250"))
    : [];
...
module.exports.updateListing = async (req, res) => {
  ...
  if (req.files && req.files.image && req.files.image.length > 0) {
    const newImage = req.files.image.map(file => ({
      url: file.path,
      filename: file.filename,
    }));
    updatedListing.image = newImage; // Update images with the new ones
