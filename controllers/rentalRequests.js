...
module.exports.checkoutListing = async (req, res) => {
  try {
    ...
    // Get Rental Request for this listing and customer
    const rentalRequest = await RentalRequest.findOne({ listing_id: listingId, customer_id: customerId });

    if (!rentalRequest) return res.status(404).send("Rental request not found.");

    const start = new Date(rentalRequest.rental_start_date);
    const end = new Date(rentalRequest.rental_end_date);
    const rentalDays = Math.ceil((end - start) / (1000 * 60 * 60 * 24)); // Difference in days
